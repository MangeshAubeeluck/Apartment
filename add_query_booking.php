<?php
	...
	if(ISSET($_POST['add_guest'])){
		...
		$query2 = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$checkin' && `Apartment_id` = '$_REQUEST[Apartment_id]' && `status` = 'Pending'") or die(mysqli_error());
	    $row = $query2->num_rows;
				...
				if($row > 0){
					echo "<div class = 'col-md-4'>
						...
						if($guest_id = $fetch['guest_id']){
							$Apartment_id = $_REQUEST['Apartment_id'];
							$conn->query("INSERT INTO `transaction`(guest_id, Apartment_id, status, checkin) VALUES('$guest_id', '$Apartment_id', 'Pending', '$checkin')") or die(mysqli_error());
